"""
Context formatting utilities for Shakespeare RAG chatbot.
...
def format_play_result(result: Dict[str, Any]) -> str:
...
def format_sonnet_result(result: Dict[str, Any]) -> str:
...
def format_poem_result(result: Dict[str, Any]) -> str:
...
def format_search_results(results: Dict[str, List[Dict[str, Any]]]) -> str:
    ...
    for collection_type, collection_results in results.items():
        ...
        for i, result in enumerate(collection_results, 1):
            if collection_type == "plays":
                formatted_result = format_play_result(result)
            elif collection_type == "sonnets":
                formatted_result = format_sonnet_result(result)
            elif collection_type == "poems":
                formatted_result = format_poem_result(result)
            else:
...
def build_context_from_results(query: str, search_results: Dict[str, List[Dict[str, Any]]]) -> Tuple[List[str], List[str]]:
        ...
        for i, result in enumerate(results[:3]):  # Limit to top 3 per collection
            ...
            if content:
                ...
                if collection_type == "plays":
                    ...
                    enhanced_content = _build_enhanced_content(content, result.get("context", {}))
...
def _build_enhanced_content(main_content: str, context_data: Dict[str, Any]) -> str:
...
def analyze_search_quality(search_results: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
